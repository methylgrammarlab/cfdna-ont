...
best_hypo_hyper_probes_file <- paste0( "/vol/sci/bio/data/benjamin.berman/ekushele/filippo/data/josh_moss_probes/creating_new_atlas_scripts/CpGs.100bp-block.",num_selected_cpgs/2,".X1.xls")
...
# }
pvalue=as.numeric(args[2])# 0.01
print(pvalue)
diff_mean=as.numeric(args[3]) #0.3
print(diff_mean)
...
#samples_mapped_to_CpG <- dir(file.path("/vol/sci/bio/data/benjamin.berman/ekushele/filippo/data/samples_nanopore/","samples_merged_CpG_450K"),full.names=T,pattern=".hg38_shifted_check.bed")
#samples_mapped_to_CpG <- dir(file.path("/vol/sci/bio/data/benjamin.berman/ekushele/filippo/data/samples_nanopore/","samples_merged_CpG_450K"),full.names=T,pattern="_unique_for_new_file.bed")


all_table_methylation_in_tissue <- do.call(rbind,lapply(samples_mapped_to_CpG,function(sample_bed) {
	print(sample_bed)
	sample_mapped <- data.frame(fread(sample_bed))
...
# tumor_fractions <- data.frame(read.table(text=system("less -S /vol/sci/bio/data/benjamin.berman/ekushele/filippo/data/tumor_fractions/tumorFracTable_subDT_percCNA_015_maxCN3setto0_with_num_reads.txt| awk -F '\t' -v OFS='\t' '{print $1,$8,$20}'",intern=T),header=T))
